...
public class ProfileScoreStrategy implements ScoreStrategy {
    ...
    public Map<Event, Double> computeScores(List<Event> events) {
        log.info("ProfileScoreStrategy::computeScores: computing scores for {} events with user profile {}", events.size(), user);
    ...
    private double generateEventScore(Event event) {
        ...
        log.info("ProfileScoreStrategy::generateEventScore: user has sports of preference, calculating score for event {}", event);
